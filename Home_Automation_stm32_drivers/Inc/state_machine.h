...
typedef enum {
   ...
   BEEP_ERROR,              // 2 short beeps (200ms each)
   BEEP_WARNING,            // 3 short beeps 200ms each
   BEEP_ALARM,              // 5 short 200 ms each
...
//
//// Add these core function prototypes
//uint32_t SysTick_Config(uint32_t ticks);
//void NVIC_SetPriority(int32_t IRQn, uint32_t priority);

///* ===== Function Prototypes ===== */
//
uint8_t System_SelfTest(void);           // ← ADDED
