...
/*
 * Returns the System Core Clock (SYSCLK) Frequency in Hz
 * This IS the processor speed.
 */
uint32_t RCC_GetSystemClock(void)
{
    uint32_t systemClk = 0;
    uint8_t clksrc = (RCC->CFGR >> 2) & 0x3; // Read SWS bits (System Clock Switch Status)

    if(clksrc == 0)
    {
        // System is running on HSI (Internal High Speed RC)
        systemClk = HSI_CLK_VALUE; // Usually 16,000,000 Hz
    }
    else if(clksrc == 1)
    {
        // System is running on HSE (External Crystal/Oscillator)
        systemClk = HSE_CLK_VALUE; // Defines in your header, e.g., 8,000,000
    }
    else if(clksrc == 2)
    {
        // System is running on PLL
        systemClk = RCC_GetPLLOutputClock();
    }
